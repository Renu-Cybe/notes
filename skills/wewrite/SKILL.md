...
**度量收集**：在 Step 1 开始时初始化收集器，各步骤边界记录耗时，Step 8 结束时保存数据。

```python
# Step 1 开始时
from metrics.collector import start_collection
collector = start_collection()
collector.record("trigger_type", "manual")  # 根据实际触发方式
collector.record("user_query", user_input)

# 各步骤边界（示例）
collector.step_start("topic_selection")  # Step 2 开始
collector.step_end("topic_selection")    # Step 2 结束

# Step 8 完成时
collector.record("word_count", word_count)
collector.record("published", published)
collector.record("pass_on_first_try", pass_on_first_try)
collector.finalize()  # 保存数据
```
...
检查: topics 是否包含 "AI" / "人工智能" / "科技" / "builder"
检查: enable_builder_linkage == true
```

如果满足联动条件 → 调用 `follow-builders` skill 获取最新 AI builder 动态：
- 合并到选题池
- AI相关选题优先使用 builder素材作为真实锚点
...
**失败处理**：WebSearch 无结果或不可用 → 告知用户"无法获取最新素材，请检查网络或稍后重试"，停止流程，不降级到训练数据。
