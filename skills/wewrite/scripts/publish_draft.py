...
# Shorten title if needed (WeChat limit: 5-64 UTF-8 bytes)
title = "AI助手入门：写出你的第一个工具"  # Fixed shorter title

# Remove edit anchors and tags section
...
if len(summary.encode('utf-8')) > 120:
    summary = summary[:50]  # Truncate if too long

print('Title:', title)
print('Title bytes:', len(title.encode('utf-8')))
print('Summary bytes:', len(summary.encode('utf-8')))
...
resp = requests.post(url, data=json.dumps(data, ensure_ascii=False).encode('utf-8'), headers={'Content-Type': 'application/json; charset=utf-8'}, timeout=30)
result = resp.json()
